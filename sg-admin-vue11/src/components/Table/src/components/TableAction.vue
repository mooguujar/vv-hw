  ...
  <div :class="[prefixCls, getAlign]" @click="onCellClick">
    <template v-for="(action, index) in getActions" :key="`${index}-${action.label}`">
      ...
      <template v-else>
        <Tooltip v-if="action.tooltip" v-bind="getTooltip(action.tooltip)">
          <PopConfirmButton v-bind="action" :class="action.class">
    ...
    >
      ...
      <a-button type="link" size="small" v-if="!$slots.more">
        {{ t('common.more') }} <Icon icon="mdi-light:chevron-down" />
      </a-button>
    </Dropdown>
  </div>
</template>
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      return { prefixCls, getActions, getDropdownList, getAlign, onCellClick, getTooltip, t };
    },
  });
</script>
<style lang="less">
  ...
  .btn-40px {
    button {
      ...
      &.ant-btn-error {
        border: 1px solid #e91134 !important;
