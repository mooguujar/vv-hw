  ...
  <div :class="prefixCls">
    ...
    >
      <div class="cell-content" :title="column.ellipsis ? getValues ?? '' : ''">
        <div v-if="column.dataIndex === 'effective_amount'">
          <cdIconCurrency class="w-22px 444" :icon="getCurrency" />
          {{ getValues ? getValues : '&nbsp;' }}</div
        >
        <div v-else-if="column.dataIndex === 'config_amount'">
          <cdIconCurrency class="w-22px 222" :icon="getCurrency" />
          {{ getValues ? getValues : '&nbsp;' }}</div
        >
        <div v-else> {{ getValues ? getValues : '&nbsp;' }}</div>
      </div>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const { getCurrency } = useUserStore();
