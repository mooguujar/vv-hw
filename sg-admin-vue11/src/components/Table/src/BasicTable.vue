      ...
      >
        ...
        <template #bodyCell="data">
          ...
          <template v-if="data.column.slotsBak?.customRender">
            <div
              :style="{
                width: Math.max(data.column.width, getColumnWidth(data.column.customTitle)) + 'px',
                minWidth:
                  Math.max(data.column.minWidth, getColumnWidth(data.column.customTitle)) + 'px',
              }"
            >
              <slot :name="data.column.slotsBak.customRender" v-bind="data || {}"></slot>
            </div>
          </template>
          <template v-else-if="data.column?.customRender">
            <div
              :style="{
                width: Math.max(data.column.width, getColumnWidth(data.column.customTitle)) + 'px',
                minWidth:
              ...
              ><value-node :func="data.column?.customRender" :scope="data"
            /></div>
          </template>
          <template v-else>
            <div
            ...
            >
              <!-- {{ data.column.width + '-' + getColumnWidth(data.column.customTitle) }} -->
              ...
              :style="{
                width: !data.column.width
                  ? '100%'
                  : isString(data.column.width)
                  ? data.column.width
                  : parseFloat(data.column.width) - 16 + 'px',
                minWidth: !data.column.minWidth
                  ? ''
                  : isString(data.column.minWidth)
                  ? data.column.minWidth
                  : parseFloat(data.column.minWidth) - 16 + 'px',
              }"
            >
              ? data.column.width : parseFloat(data.column.width) - 16 + 'px', -->
              <!-- {{ data.column.width + '-' + getColumnWidth(data.column.customTitle) }} -->
              {{
                isValid(data.record[data.column.dataIndex])
...
<script lang="ts">
  ...
  import { defineComponent, ref, computed, unref, toRaw, inject, watchEffect, h } from 'vue';
  import { Table, FormItemRest } from 'ant-design-vue';
  ...
  import { isFunction, isString, isValid } from '/@/utils/is';
  import { warn } from '/@/utils/log';

  function RenderFun(props) {
    return props.func(props.scope);
  }
  RenderFun.props = ['func', 'scope'];
    ...
    setup(props, { attrs, emit, slots, expose }) {
      ...
      const getBindValues = computed(() => {
        ...
        let propsData: Recordable = {
          ...
          loading: getProps.value.showLoading === false ? false : unref(getLoading),
          tableLayout: 'fixed',
...
<style lang="less">
  ...
  .@{prefix-cls} {
    ...
    .ant-table {
      ...
      //定义行颜色
      .trcolor {
        background-color: rgb(255 192 203 / 31%);
        color: '#E91134';
      }
