...
export function useCustomSelection(
  ...
  const selectHeaderProps = computed(() => {
    return {
      ...
      selectedLength: flattedData.value.filter((data) =>
        selectedKeys.value.includes(getRecordKey(data)),
      ).length,
  ...
  function onSelectAll(checked: boolean) {
    ...
    const checkAll = () => {
      if (modal != null) {
        modal.update({
          content: t('common.translate.ple_wait'),
    ...
    if (flattedData.value.length > 120) {
      modal = createConfirm({
        title: t('business.common_select_all'),
        content: t('common.translate.operate_tips'),
        iconType: 'warning',
        onOk: () => checkAll(),
      });
    } else {
  ...
  function recordIsShow(index: number, threshold = 3) {
    ...
    if (isVirtual) {
      ...
      const flag2 =
        index * rowHeight.value < scrollTop.value + bodyHeight.value + rowHeight.value * threshold;
      // 全部条件满足时，才显示当前行
      return flag1 && flag2;
    }
    return true;
  ...
  function handleCustomSelectColumn(columns: BasicColumn[]) {
    ...
    columns.unshift({
      title: t('common.translate.selectColumns'),
      flag: 'CHECKBOX',
