import type { ActionResult, BasicTableProps, FetchParams, SorterResult } from '../types/table';
...
export function useDataSource(
  ...
  function handleTableChange(
    ...
    if (action.action === 'sort' && pagination.current != 1) {
  ...
  async function fetch(opt?: FetchParams) {
    ...
    try {
      ...
      if (pageParams[pageField] === 1) {
        // 后台只在第一次请求返回总条数，故在此判断，无需要可删除
        setPagination({
