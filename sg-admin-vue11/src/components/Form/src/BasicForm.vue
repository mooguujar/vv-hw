<template>
  ...
  >
    <Row v-bind="getRow" class="t-form-col">
    ...
    setup(props, { emit, attrs }) {
        ...
        for (const schema of schemas) {
          ...
          if (isHandleDateDefaultValue && defaultValue && dateItemType.includes(component)) {
            ...
            if (!Array.isArray(defaultValue)) {
              const date = defaultValue.format
                ? defaultValue
                : dateUtil(defaultValue).isValid()
                ? dateUtil(defaultValue)
                : dateUtil();
              schema.defaultValue = valueFormat ? date.format(valueFormat) : date;
            } else {
              const def: any[] = [];
              defaultValue.forEach((item) => {
                const date = item.format
                  ? item
                  : dateUtil(item).isValid()
                  ? dateUtil(item)
                  : dateUtil();
                def.push(valueFormat ? date.format(valueFormat) : date);
              });
      ...
      watch(
        ...
        (schema) => {
          ...
          nextTick(() => {
            useAutoLabelWidth(getSchema.value);
          });
