...
export function useFormEvents({
  ...
  async function setFieldsValue(values: Recordable): Promise<void> {
    ...
    fields.forEach((key) => {
      ...
      if (typeof componentProps === 'function') {
        // setFieldsValue Cannot destructure property of 'formActionType' as it is undefined
        // https://github.com/vbenjs/vue-vben-admin/issues/2737
        formElRef.value = {
          ...formElRef.value,
          setFieldsValue,
          removeSchemaByField,
  ...
  async function updateSchema(data: Partial<FormSchema> | Partial<FormSchema>[], noUseUniqBy: boolean = true) {
    let updateData: Partial<FormSchema>[] = [];
    if (isObject(data)) {
      updateData.push(data as FormSchema);
    }
    ...
    if (noUseUniqBy) {
      ...
      // TODO: 为了解决调用updateSchema()ifshow字段不生效的问题，暂时使用uniqBy()方法
