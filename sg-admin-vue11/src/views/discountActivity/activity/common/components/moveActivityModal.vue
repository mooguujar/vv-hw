<template>
  <BasicModal
    ...
    :title="t('table.discountActivity.move_activity')"
    @ok="handleSubmit"
  >
    <div class="flex flex-row items-center justify-center">
      <Icon
        ...
        theme="filled"
        style="color: #faad14"
      /><span class="text-center text-lg">{{t('common.translate.word9')}}【{{ activityName }}】</span>
    </div>
  </BasicModal>
</template>
...
<script lang="ts" setup>
  ...
  const [registerBasicModal, { closeModal }] = useModalInner((data) => {
    activityName.value = data.record.zh_name;
    promoId.value = data.record.id;
