...
<template>
  <PageWrapper :contentStyle="{ margin: '0px' }">
    ...
    <basic-config
      ...
      :timedIdisable="timedIdisable"
    />
    <div v-if="activeConfig === 1">
      <Row>
        <div class="flex">
          <div class="w-200px mr-10px"
            >{{ t('v.discount.activity.BonusType1') }}
            <cdIconCurrency :icon="currencyName" class="w-5 mb-1 ml-1" />
          </div>
          <div>
            <Col :span="24">
              <RadioGroup
                v-model:value="conditionData[currentConditionData][0].type"
                name="radioGroup"
                :disabled="!!timedIdisable"
              >
                <Radio value="1">{{ t('v.discount.activity.BonusType2') }}</Radio>
                <Radio value="2">{{ t('v.discount.activity.BonusType3') }}</Radio>
              </RadioGroup>
            </Col>
          </div>
        </div>
      ...
      <Row class="mt-20px">
        <div class="w-90px mr-10px"
          >{{ t('common.Departmental_scholarships') }}
          <cdIconCurrency :icon="currencyName" class="w-5 mb-1 ml-1" />
        </div>
        <div style="flex: 1">
          <Col :span="24">
            <dollar-condition
              ...
              v-if="conditionData[currentConditionData][0].type == '1'"
              ...
              v-model:currencyName="currencyName"
              v-model:bonusType="conditionData[currentConditionData][0].type"
              :timedIdisable="timedIdisable"
              style="width: 100%"
            />
            <dollar-condition
              ref="dollarConditionRef"
              v-if="conditionData[currentConditionData][0].type == '2'"
              v-model="conditionData[currentConditionData]"
              v-model:condition-type="conditionType"
              v-model:conditionTime="conditionTime"
              v-model:firstCurrencyId="firstCurrencyId"
