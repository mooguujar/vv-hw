<script lang="ts" setup name="DollarWaves">
  ...
  function updateOtherLang(langList) {
    if (props.type == 'rechargeDay' || props.type == 'memberDay') return false;
    langList.forEach((lang) => {
  ...
  watch(
    ...
    () => {
      ...
      const rechargeMoney = langKeys.reduce((pre, item) => {
        const current = conditionData.value[item];
        const rewardList = current.map((r) => {
          const reward = Number(r.miniDeposit);
          return isNaN(reward) ? 0 : reward;
        });
        pre[item] = Math.min(...rewardList);
        return pre;
      }, {});

      const rechargeMoney13 = langKeys.reduce((pre, item) => {
        const current = conditionData.value[item];
        const rewardList = current.map((r) => {
          const reward = Number(r.miniDeposit);
          return isNaN(reward) ? 0 : reward;
      ...
      const rechargeBet13 = langKeys.reduce((pre, item) => {
        const current = conditionData.value[item];

        // 找到 miniDeposit 最大值对应的对象
        const maxRewardItem = current.reduce((maxItem, r) => {
          const reward = Number(r.miniDeposit);
          if (!isNaN(reward) && (maxItem === null || reward > Number(maxItem.miniDeposit))) {
            return r;
          }
          return maxItem;
        }, null);

        // 获取对应的 everyReward，如果没有则设为 0
        pre[item] = maxRewardItem ? Number(maxRewardItem.everyReward) : 0;
        return pre;
      }, {});

      eventBus.emit('onEvertMemberTextChange', { value: currentMoney, type: 'memberDayaward' });
      eventBus.emit('onEvertMemberTextChange', { value: rechargeMoney, type: 'memberRecharge' });

      eventBus.emit('onEvertMemberTextChange', { value: rechargeMoney13, type: 'rechargeMoney13' });
      eventBus.emit('onEvertMemberTextChange', { value: rechargeBet13, type: 'rechargeBet13' });
    },
    { deep: true },
  );
  defineExpose({
