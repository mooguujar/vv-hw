  ...
  <div>
    ...
    >
      ...
      <template #action="{ record }">
        <span class="primary-color cursor" @click="deletFun(record)"
...
<script lang="ts" setup>
  ...
  import { updateLowList } from '/@/api/risk/index';
  ...
  function deletFun(record) {
    openConfirm(t('common.warning'), t('modalForm.risk.risk_delete_tip'), async () => {
      try {
        const { status, data } = await updateLowList({ id: record.id, limit_type: 4 });
        if (status) {
