  ...
  <div>
    <BasicTable @register="registerTable" class="!p-0 mb-14px">
      <template #form-created>
        <div class="w-full" style="width: 100vw">
          <RadioGroup v-model:value="tabValue" button-style="solid" class="!ml-3">
            <RadioButton value="gtag" class="button-group-btn"
              >GTAG({{ $t('table.promotion.promotion_recommended') }})</RadioButton
            >
            <RadioButton value="gtm" class="button-group-btn"
              >GTM({{ $t('table.promotion.promotion_simular_code_type') }})</RadioButton
            >
          </RadioGroup>
        </div>
      </template>
      ...
      <template #customTableHeader v-if="isHasAuth('30102')">
        <div class="gtag_gtm mb-12px">
          <div class="gtag_gtm_box">
            <div>
              ...
              >
                <template #addonBefore>
                  <Select v-model:value="domain" style="width: 120px">
            ...
            <div>
              <p class="gtag_gtm_box_title" v-if="tabValue === 'gtag'">
                <span>GTAG ID({{ $t('table.promotion.promotion_from_google') }}.</span
                ><span class="primary-color cursor">{{
                  $t('table.promotion.promotion_detail_lessons')
                }}</span
                ><span>)</span>
              </p>
              <p class="gtag_gtm_box_title" v-if="tabValue === 'gtm'">
                <span>GTM ID({{ $t('table.promotion.promotion_from_google') }}.</span
                ><span class="primary-color cursor">{{
                  $t('table.promotion.promotion_detail_lessons')
                }}</span
                ><span>)</span>
    ...
    <Specification :tabValue="tabValue" />
  </div>
</template>
...
<script lang="ts" setup>
  ...
  const labelWidth = (() => {
    switch (locale) {
      case 'zh_CN':
        return '95px';
      case 'en_US':
        return '-185px'; // 修改为返回 'auto' 而不是 120
      case 'vi_VN':
        return '-25px'; // 修改为返回 'auto' 而不是 120
      case 'pt_BR':
        return '-55px'; // 修改为返回 'auto' 而不是 120
      default:
        return '0px';
    }
  })();
  ...
  const { t } = useI18n();
  const tabValue = ref<string>('gtag');
  ...
  async function createBuriedPoint() {
    ...
    if (!track_id.value) {
      message.error(t('table.promotion.promotion_customer_room_p_id'));
      return false;
    }
