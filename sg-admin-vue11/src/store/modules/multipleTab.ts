...
export const useMultipleTabStore = defineStore({
  ...
  actions: {
    ...
    async updateCacheTab() {
      ...
      //将临时缓存添加到缓存路由列表
      Array.from(this.tempCatcheMenu).forEach((item) => {
        cacheMap.add(item);
      });
    ...
    async closeTab(tab: RouteLocationNormalized, router: Router) {
      // 'DetailsMember' || name == 'DetailsOnlineMember'?
      this.tabList = this.tabList.filter(
        (item) => item.name != 'DetailsMember' && item.name != 'DetailsOnlineMember',
      );
