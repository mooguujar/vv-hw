...
defineOptions({ name: 'BaseScrollPicker' })
...
function setScroll(scroll: number) {
  ...
  scrollRaf = requestAnimationFrame(() => {
    if (rotatorRef.value) {
      rotatorRef.value.style.top = `${scrollOffset - scroll - 5}px`
    }
    scrollRaf = null
  })
  return scroll
...
<template>
  <div ref="el" class="vue-scroll-picker">
    <div
      ref="rotatorRef"
      role="listbox"
      class="vue-scroll-picker-rotator" :class="[
        {
...
.vue-scroll-picker-item[aria-selected='false'] {
