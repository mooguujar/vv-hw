...
import { ApiSportEventInfo } from '@tg/apis'
import { ESportsToMainPageRoutes, EventBusNames } from '@tg/types'
import { appEventBus, application, getCartObject, getSportsLiveTime } from '@tg/utils'
...
export function useApiSportDetails() {
  ...
  const breadcrumbData = computed<IBreadCrumbItem[]>(() => {
    ...
    data.push(
      ...
      {
        title: list.pgn,
        path: `/sports/${list.si}/${list.pgid}?${application.objectToUrlParams({ sn: list.sn, pgn: list.pgn })}`,
        data: {
          name: ESportsToMainPageRoutes.REGION,
          data: {
            si: list.si,
            pgid: list.pgid,
            query: application.objectToUrlParams({ sn: list.sn, pgn: list.pgn }),
          },
        },
      },
      {
        title: list.cn,
        path: `/sports/${list.si}/${list.pgid}/${list.ci}?${application.objectToUrlParams({ sn: list.sn, pgn: list.pgn, cn: list.cn })}`,
        data: {
          ...
          data: {
            ...
            pgid: list.pgid,
            ci: list.ci,
            query: application.objectToUrlParams({ sn: list.sn, pgn: list.pgn, cn: list.cn }),
          },
        },
      },
      {
  ...
  const dataList = computed<ISportEventInfoMl[]>(() => {
    ...
    for (let i = 0; i < _filter.length; i++) {
      ...
      if (mlItem.pat === 1 || mlItem.pat === 3) {
        ...
        const titleList = [homeTeamName, t('和局'), awayTeamName]
      ...
      else if (mlItem.pat === 8) {
        const list0 = sportInfo.value.list[0]
        const homeTeamName = list0.htn
        const awayTeamName = list0.atn
        const titleList = [homeTeamName, awayTeamName]
        const msList = mlItem.ms
        const zList = []
        const kList = []
        for (let j = 0; j < msList.length; j++) {
