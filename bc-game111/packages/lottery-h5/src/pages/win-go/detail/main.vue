...
import AppWinGoDetailItem from '../_components/AppWinGoDetailItem.vue'

const winGoStore = useWinGoStore()
const { winGoTabArr } = storeToRefs(winGoStore)
const curTab = ref<number>(winGoTabArr.value.length > 0 ? winGoTabArr.value[0].value : 1001)
const page = ref(1)
const total = ref(1)
...
async function init() {
  if (!isIFrame() && !isFlutterApp() && winGoTabArr.value.length < 1) {
    await getNav()
  }
  if (isLogin.value) {
...
<template>
  <div>
    <div class="mb-[16rem] px-[11rem]">
      <LotteryTabs v-model="curTab" :tabs="winGoTabArr" @update:model-value="onTabChange" />
    </div>
    <div v-if="isLogin && sourceData.length > 0" class="rounded-[8rem] overflow-hidden mx-[12rem] bg-white">
      <div class="mx-[13rem]">
        <AppWinGoDetailItem :data="sourceData" :is-clear="false" />
