<script setup lang="ts" name="AppTrxWinGoBet">
...
const { $$t } = useLocale()
const { trxWinGoTabArr } = storeToRefs(useTrxWinGoStore())
...
const currentKind = computed(() => trxWinGoTabArr.value.filter(item => item.value === props.currentTab)[0]?.label)
    ...
    bets: [
      {
        id: betId.value,
        play_id,
        bet_balls: play_id === 501 ? JSON.stringify([Number(props.target.text)]) : '[]',
        odds: props.betOdd,
