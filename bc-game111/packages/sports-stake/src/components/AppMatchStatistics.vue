...
<template>
  <div class="app-match-statistics wrapper has-background" :class="{ round }" :title="formatTitleData(_data)">
    <div
      ...
      }" style="--ss-sport-image-error-icon-size: 14rem"
    >
      <!-- competitor -->
      ...
      <div class="sticky-left competitor-item with-border" style="grid-area: competitor_home;">
        <template v-if="showSkeleton">
          <SSBaseSkeleton width="20rem" height="20rem" style="--ss-skeleton-border-radius:3rem" />
          <SSBaseSkeleton style="margin-left: 8rem;--ss-skeleton-border-radius:3rem" width="100rem" height="16rem" />
        </template>
        <template v-else>
...
<style lang="scss">
:root {
  --ss-app-match-statistics-max-width: calc(100% - 28rem);
}
</style>
...
.app-match-statistics {
  min-width: 250rem;
  max-width: var(--ss-app-match-statistics-max-width);
  width: auto;
}
