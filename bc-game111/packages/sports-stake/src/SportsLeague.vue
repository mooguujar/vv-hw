...
import AppSportsLevel3LiveUpcoming from './components/AppSportsLevel3LiveUpcoming.vue'
...
defineOptions({ name: 'StakeSportsLeague' })
...
const breadcrumb = computed(() => [
  ...
  {
    path: `/sports/${sport.value}/${region}?${application.objectToUrlParams({ sn: navObj.sn, pgn: navObj.pgn })}`,
    title: navObj.pgn,
    data: {
      ...
      data: {
        si: sport.value,
        pgid: region,
        query: application.objectToUrlParams({ sn: navObj.sn, pgn: navObj.pgn }),
      },
    },
  },
  {
  ...
  <div class="tg-sports-category-index">
    <div class="wrapper">
      ...
      <div class="tab-box">
        <div class="left">
          <SSBaseTabs
            v-model="curTab" class="theme-tab" :list="tabs" :center="false"
            @change="isFirstFalse"
          />
        </div>
      </div>
      <AppSportsMarketTypeSelect
        v-show="isLiveAndUpcoming" v-model="baseType" v-model:is-standard="isStandard"
      ...
      <template v-if="isFirst">
        <!-- 滚球及即将开赛 -->
        <AppSportsLevel3LiveUpcoming v-if="isLiveAndUpcoming" :base-type="baseType" :is-standard="isStandard" />
        <!-- 冠军 -->
